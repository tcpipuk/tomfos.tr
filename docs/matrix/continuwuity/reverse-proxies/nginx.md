...
First, set up an upstream definition for Continuwuity. Using Unix sockets is recommended when both
Nginx and Continuwuity are on the same machine for improved performance:

```nginx title="/etc/nginx/conf.d/upstream.conf"
upstream continuwuity_server {
    server unix:/run/continuwuity/continuwuity.sock max_fails=0;
...
```nginx title="conf.d/server.name.conf"
...
```nginx title="conf.d/matrix.server.name.conf"
# Matrix homeserver
server {
